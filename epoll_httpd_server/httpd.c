...
static int excu_cgi(int sock,const char* method, char* path,const char* query_string)
{
	...
	{
		clear_header(sock);
		//GET方法的参数跟在URL的后边，此时已经读取到，因此只需要将请求报头读完就可以回响了
		//POST方法的参数在请求报头中，而参数的多少由Content-Length所标识
	}
	else
	{
...
int handle(int sock)
{
	...
	if(strcasecmp("GET",method) == 0) //strcasecmp表示忽略大小写的比较
	{
		query_string = url;
